...
const generatedGoHeader = "// Code generated by GCO ORM. DO NOT EDIT."
...
func Run(args []string) error {
	...
	if err := removeStaleGeneratedFiles(outputDir, genFiles); err != nil {
		return err
	}
...
func removeStaleGeneratedFiles(outputDir string, genFiles []*golang.GeneratedFile) error {
	expected := make(map[string]bool, len(genFiles))
	for _, f := range genFiles {
		expected[filepath.Clean(f.Path)] = true
	}
	...
	return filepath.WalkDir(outputDir, func(path string, d os.DirEntry, err error) error {
		...
		if expected[filepath.Clean(rel)] || filepath.Ext(path) != ".go" {
			return nil
		}
...
func isGeneratedGoFile(path string) bool {
	...
	return strings.HasPrefix(string(data), generatedGoHeader)
}
...
func buildGuideDoc(schema *ir.Schema) string {
	...
	b.WriteString("## Bulk Inserts\n\n")
	b.WriteString("Use `BulkCreate` for high-volume inserts, idempotent sync jobs, and join table writes:\n\n")
	b.WriteString("```go\n")
	b.WriteString("rows, err := c.Post.BulkCreate(posts).\n")
	b.WriteString("    OnConflictDoNothing(query.PostIdColumn).\n")
	b.WriteString("    Returning(query.PostIdColumn).\n")
	b.WriteString("    BatchSize(1000).\n")
	b.WriteString("    DoReturningValues(ctx)\n")
	b.WriteString("```\n\n")
	b.WriteString("- `Do(ctx)` executes batches and returns affected rows.\n")
	b.WriteString("- `DoReturning(ctx)` returns inserted model rows on PostgreSQL.\n")
	b.WriteString("- `DoReturningValues(ctx)` returns selected `RETURNING` columns as row maps.\n")
	b.WriteString("- `CreateMany(ctx, data)` remains available and delegates to `BulkCreate(data).Do(ctx)`.\n\n")

	b.WriteString("## Raw SQL\n\n")
	b.WriteString("Use raw SQL for CTEs, `UPDATE ... FROM`, query-plan-sensitive joins, and PostgreSQL-specific features:\n\n")
	b.WriteString("```go\n")
	b.WriteString("posts, err := client.Raw[model.Post](ctx, c,\n")
	b.WriteString("    `SELECT id, title, author_id FROM posts WHERE id = $1`, id)\n")
	b.WriteString("```\n\n")
	b.WriteString("- `client.Raw[T](ctx, c, sql, args...)` scans rows into a struct using `db` tags.\n")
	b.WriteString("- `c.RawRows(ctx, sql, args...)` returns `[]map[string]any` for ad-hoc projections.\n")
	b.WriteString("- `c.RawExec(ctx, sql, args...)` executes custom SQL and returns affected rows.\n\n")

	b.WriteString("## Table and Column Constants\n\n")
	b.WriteString("Generated query packages include constants for hand-written SQL:\n\n")
	b.WriteString("```go\n")
	b.WriteString("query.PostTable\n")
	b.WriteString("query.PostIdColumn\n")
	b.WriteString("query.PostAuthorIdColumn\n")
	b.WriteString("```\n\n")
